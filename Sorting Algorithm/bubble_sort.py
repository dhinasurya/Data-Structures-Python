def bubble_sort(my_list):
    ...
    for i in range(len(my_list) - 1, 0, -1):  # Start from the end and reduce the range with each pass
        # Inner loop: Compares adjacent elements
        for j in range(i):  # Iterate through the unsorted portion of the list
            # Compare the current element with the next element
            if my_list[j] > my_list[j+1]:  # If the current element is greater, swap them
